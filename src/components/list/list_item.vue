<template>
    <li class="u-item">
        ...
        <div class="u-content">
            <ul class="m-macro-list-item-meta" v-if="item.post_meta && item.post_meta.data && item.post_meta.data.length">
                <li class="u-macro" v-for="(m, i) in item.post_meta.data" :key="i">
                    <img class="u-macro-icon" :src="m.icon | iconLink" />
        ...
        <div class="u-misc">
            <!-- 作者 -->
            <div class="u-author">
                <a class="u-author-name" :href="item.post_author | authorLink" target="_blank">{{ item.author }}</a>
            </div>
            <span class="u-date">
...
import { showAvatar, authorLink, showBanner, buildTarget, iconLink } from "@jx3box/jx3box-common/js/utils";
